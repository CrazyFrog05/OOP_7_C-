...
void LinkedList::Push_back( int data) {
	...
	_numberOfElements++;
	if (_head == nullptr) {
		_head = newNode; // Если в нод нет объектов, то новый объект будет и головой и хвостом
		_tail = newNode;
		return;
	}
	newNode->SetNext(_tail);
	_tail = newNode; // Происзодит сдвиг объекта и получается, что добавляем объект, а сам хвост становится обычным объетом
}
